/**
 * Test command router script
 * Routes to the appropriate test script based on content type
 */
import fs from "fs";
...
async function main() {
  try {
    ...
    const scriptPath = contentType === "products" ? productTestScript : categoryTestScript;
    
    // Check if the script exists
    if (!fs.existsSync(scriptPath)) {
      console.error(`Test script for ${contentType} not found at: ${scriptPath}`);
      process.exit(1);
    }
